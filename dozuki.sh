...
# Need to manually fix up iFixit-Info.plist
   ...
   sed -i '.bak' -e 's/com.dozuki.dozuki/com.dozuki.dripassist/g' iFixit-Info.plist
   sed -i '.bak' -e 's/Dozuki/DripAssistant/g' iFixit-Info.plist
   sed -i '.bak' -e 's/>dozuki</>dripassist</g' iFixit-Info.plist
   sed -i '.bak' -e 's/\[Config currentConfig\].dozuki = YES;/\[Config currentConfig\].dozuki = NO;/' Classes/iFixitAppDelegate.m
   sed -i '.bak' -e 's/\[Config currentConfig\].site = ConfigDozuki;/\[Config currentConfig\].site = ConfigDripAssist;/' Classes/iFixitAppDelegate.m
